...
![Cross-Validated-Score-Diagram](/images/posts/20190311/Cross-Validated-Score-Diagram.jpg)
...
## 选择胜出的模型
现在，通过交叉验证的调优，每种算法有了一个最优模型。最重要的是，到目前为止，你仅仅只是用了训练数据。现在，是时候评估每种模型并选择最优的一个，就像饥饿游戏的模式一样。因为你已经将你的测试集保存为真正的未知数据，现在你可以用它来评估每个模型性能的可靠性了。
